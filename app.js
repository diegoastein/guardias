...
function App() {
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center bg-white p-3 rounded shadow-sm">
                    ...
                    <div className="flex gap-2">
                        <Button onClick={() => {if(confirm('¿Aplicar plantilla?')) {const n={}; for(let d=1; d<=days; d++){const f=new Date(year,month,d), t=plantilla[f.getDay()]; if(t) n[f.toISOString().split('T')[0].slice(0,10)]=t;} dbBatchGuardias(n);}} variant="secondary">Plantilla</Button>
                    </div>
                </div>
                ...
                {selectedDay && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <Card className="w-full max-w-2xl overflow-y-auto max-h-[90vh]">
                        <div className="flex justify-between mb-4"><h3 className="font-bold">Editar {formatDate(selectedDay, month, year)}</h3><button onClick={()=>setSelectedDay(null)}><XCircle/></button></div>
                            ...
                            {['Día 1', 'Día 2', 'Noche 1', 'Noche 2'].map((l, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-gray-50 p-2 border rounded">
                                        <div className="font-bold text-xs mb-1">{l}</div>
    ...
    // --- VISTA LICENCIAS (MODIFICADA SEGÚN PEDIDO) ---
                        ...
                        {historial.map(lic => (
                            <li key={lic.id} className="py-4">
                                ...
                                <div className="mt-2 p-3 bg-blue-50 border border-blue-100 rounded text-sm">
                                    <p className="text-blue-800 font-bold uppercase text-[10px] mb-1">Detalle Cobertura:</p>
    ...
    function VistaReporte() {
        ...
        for(let d=1; d<=31; d++){
            ...
            const check = (s, t) => {
                ...
                if(!r) return;
                const item = {fecha:formatDate(d,month,year), receptor:r.nombre, sadofe:fObj.getDay()===0||fObj.getDay()===6};
                if(!t?.medicoId) { (vacantes[fObj.getDay()] = vacantes[fObj.getDay()] || []).push(item); }
                else if(String(t.medicoId)!==String(s.medicoId)) {
                    const tit = medicos.find(x=>String(x.id)===String(t.medicoId))?.nombre || 'Titular';
                    (licenciasGrupo[tit] = licenciasGrupo[tit] || []).push(item);
                }
            };
        ...
        return (
            <div id="reporte-ministerio" className="bg-white p-6 space-y-8">
                ...
                <div className="space-y-6">
                    <h3 className="font-bold border-b pb-1">VACANTES</h3>
                    {Object.entries(vacantes).map(([dia, items], i) => (
                        <table key={i} className="w-full text-xs border-collapse border border-gray-400">
                            ...
                            <tbody>{items.map((it, idx)=>(<tr key={idx}><td className="border p-1">{it.fecha}</td><td className="border p-1 text-center">{!it.sadofe?'X':''}</td><td className="border p-1 text-center">{it.sadofe?'X':''}</td><td className="border p-1">{it.receptor}</td></tr>))}</tbody>
                        </table>
                    ))}
                    <h3 className="font-bold border-b pb-1">LICENCIAS</h3>
                    {Object.entries(licenciasGrupo).map(([tit, items], i) => (
                        <div key={i} className="space-y-1">
                            <p className="text-xs font-bold uppercase">Licencia {tit}</p>
    ...
    function VistaBusqueda() {
        ...
        const generarMasivos = async () => {
            setLoadingPdf(true); const files = [];
            ...
            for(let id of selMedicos) {
                ...
                for(let d=1; d<=days; d++){
                    ...
                    [g.dia1, g.dia2, g.noche1, g.noche2].forEach((s,i)=>{
                        ...
                        if(cobrador === String(id) && String(titular)!==String(id)) count++;
                    });
                }
                files.push({nombre:med.nombre, total:count, excedido:count>10});
            }
            setGenResult(files); setLoadingPdf(false);
        };

        return (
            <Card>
                <div className="flex gap-4 mb-6 border-b pb-2">
                    <button onClick={()=>setMode('masivo')} className={`font-bold ${mode==='masivo'?'text-blue-600 border-b-2 border-blue-600':''}`}>Reportes Masivos</button>
                    <button onClick={()=>setMode('derivadas')} className={`font-bold ${mode==='derivadas'?'text-blue-600 border-b-2 border-blue-600':''}`}>Búsqueda Derivadas</button>
                </div>
                {mode === 'masivo' && <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-4 border rounded h-48 overflow-y-auto grid grid-cols-2 md:grid-cols-3 gap-2">
                    ...
                    <Button onClick={generarMasivos} disabled={loadingPdf}>{loadingPdf?'Generando...':'Verificar Topes'}</Button>
                    {genResult.length > 0 && <div className="mt-6 space-y-2">
                        {genResult.map((r,i)=>(<div key={i} className={`p-3 border rounded flex justify-between ${r.excedido?'bg-red-50 border-red-200 text-red-700':'bg-green-50'}`}><span>{r.nombre}</span><span className="font-bold">{r.total} u. {r.excedido?'(EXCEDE)':''}</span></div>))}
                    </div>}
                </div>}
            </Card>
        );
    ...
    function VistaConfig() {
        ...
        return (
            <div className="space-y-6">
                <Card>
                    <h3 className="font-bold mb-4">Plantilla Base</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'].map((d,i)=>(<button key={i} onClick={()=>setEditing(i)} className="p-2 bg-gray-100 rounded text-sm hover:bg-gray-200 border">{d}</button>))}
                    </div>
                </Card>
                <Card>
                    <h3 className="font-bold mb-4">Listado de Médicos</h3>
                    <table className="w-full text-sm"><tbody>{medicosOrdenados.map(m=>(<tr key={m.id} className="border-b"><td className="p-2">{m.nombre}</td><td className="p-2 text-right"><button onClick={()=>dbBorrarMedico(m.id)} className="text-red-500"><Trash2/></button></td></tr>))}</tbody></table>
                </Card>
                {editing !== null && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <Card className="w-full max-w-lg">
                        <h3 className="font-bold mb-4">Editar Plantilla: {['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'][editing]}</h3>
                        {['dia1','dia2','noche1','noche2'].map(k=>(
                            <div key={k} className="mb-2">
                                <label className="text-xs uppercase font-bold text-gray-500">{k}</label>
                                <select className="w-full text-sm" value={plantilla[editing]?.[k]?.medicoId||''} onChange={e=>dbGuardarPlantilla(editing, {...(plantilla[editing]||{}), [k]:{medicoId:e.target.value}})}>
                                    <option value="">Vacante...</option>
                                    {medicosOrdenados.map(m=><option key={m.id} value={m.id}>{m.nombre}</option>)}
                                </select>
                            </div>
                        ))}
                        <div className="mt-4 flex justify-end"><Button onClick={()=>setEditing(null)}>Guardar</Button></div>
                    </Card>
                </div>}
            </div>
        );
    ...
    if (!user) return <div className="h-screen flex items-center justify-center animate-pulse text-blue-600 font-bold">Iniciando Base de Datos...</div>;

    return (
        <div className="min-h-screen bg-gray-50 pb-20">
            <nav className="bg-white shadow p-4 mb-6 flex gap-4 overflow-x-auto">
                <button onClick={()=>setView('calendario')} className={`px-3 py-1 rounded whitespace-nowrap ${view==='calendario'?'bg-blue-100 text-blue-700 font-bold':'text-gray-500'}`}>Calendario</button>
                <button onClick={()=>setView('licencias')} className={`px-3 py-1 rounded whitespace-nowrap ${view==='licencias'?'bg-blue-100 text-blue-700 font-bold':'text-gray-500'}`}>Licencias</button>
                <button onClick={()=>setView('reporte')} className={`px-3 py-1 rounded whitespace-nowrap ${view==='reporte'?'bg-blue-100 text-blue-700 font-bold':'text-gray-500'}`}>Reporte</button>
                <button onClick={()=>setView('busqueda')} className={`px-3 py-1 rounded whitespace-nowrap ${view==='busqueda'?'bg-blue-100 text-blue-700 font-bold':'text-gray-500'}`}>Búsqueda/Masivos</button>
                <button onClick={()=>setView('config')} className="ml-auto p-1"><Settings/></button>
            </nav>
